...
mkdir -p /path/fastqtl_nominal/chr$chr

/path/fastQTL \
   --vcf  /path/geno_imputed_dosage_maf05_hg38_QC.vcf.gz \
   --bed  /path/chr${chr}_input.bed.gz   \
   --region  chr$chr \
   --out  /path/fastqtl_nominal/chr$chr/chr${chr}_output.nominal.tbru.txt

gzip -f /path/fastqtl_nominal/chr$chr/chr${chr}_output.nominal.tbru.txt
