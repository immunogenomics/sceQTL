# Script to run single-cell negative binomial mixed effects eQTL model with one cell state interaction (e.g., CV, PC, CD4+, trajectory, disease status)
...
full_model <- lme4::glmer.nb(formula = E ~ G + (1 | B) + (1 | IND) + AGE + SEX + nUMI + MT + PC1 + PC2 + PC3 + PC4 + PC5 + expPC1 + expPC2 + expPC3 + expPC4 + expPC5 + cov + G:cov, 
                             data= data, nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))
null_model <- lme4::glmer.nb(formula = E ~ G + (1 | B) + (1 | IND) + AGE + SEX + nUMI + MT + PC1 + PC2 + PC3 + PC4 + PC5 + expPC1 + expPC2 + expPC3 + expPC4 + expPC5 + cov, 
                             data= data, nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))
