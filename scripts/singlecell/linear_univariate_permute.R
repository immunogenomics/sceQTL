# Script to run single-cell linear mixed effects eQTL model with one cell state interaction (e.g., CV, PC, CD4+, trajectory, disease status) on permuted data (permuting cell state)
...
# Load interaction covariate (e.g., CD4+ binary annotation from gating on CITE-seq surface protein)
set.seed(1010)
cov <- unlist(readRDS("/path/citeseq_gates.rds")[,"cd4"])
for(x in unique(meta$donor)){
   cov[meta$donor == x] <- sample(cov[meta$donor == x])
}
...
full_model <- lme4::lmer(formula = E ~ G + (1 | B) + (1 | IND) + AGE + SEX + nUMI + MT + PC1 + PC2 + PC3 + PC4 + PC5 + expPC1 + expPC2 + expPC3 + expPC4 + expPC5 + cov + G*cov, 
                          data= data, REML = F)
null_model <- lme4::lmer(formula = E ~ G + (1 | B) + (1 | IND) + AGE + SEX + nUMI + MT + PC1 + PC2 + PC3 + PC4 + PC5 + expPC1 + expPC2 + expPC3 + expPC4 + expPC5 + cov, 
                          data= data, REML = F)
