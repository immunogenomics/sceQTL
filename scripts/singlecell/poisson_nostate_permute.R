# Script to run single-cell Poisson mixed effects eQTL model without cell state interaction on permuted data (permuting genotype)
...
G <- subset(geno, ID == snp)
colnames(G)[10:276] <- sample(colnames(G)[10:276])
G <- t(G[,as.character(meta$donor)])[,1]
...
full_model <- lme4::glmer(formula = E ~ G + (1 | IND) + (1 | B) + AGE + SEX + nUMI + MT + PC1 + PC2 + PC3 +PC4 + PC5 + expPC1 + expPC2 + expPC3 + expPC4 + expPC5, 
